$(function() {
	var api = google.maps,
		mapCenter = new api.LatLng(30.350564, 77.467313),
		mapOptions = {
			zoom: 13,
			center: mapCenter,
	...
	var addMarker = function (e) {
		if(clicks <= 1) {
			...
			var marker = new api.Marker({
				...
				icon: (clicks === 0) ? "images/start.png" : "",
				draggable: true,
	...
	$("body").on("click", "#getQuote", function (e) {
		...
		new api.DistanceMatrixService().getDistanceMatrix({
			...
			var list = $("<dl />", {
				...
				term.clone().addClass("total").text("$" + totalCost).appendTo(list);
